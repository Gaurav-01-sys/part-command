...
# from ui.ai_query_corporate import build_ai_query_tab, status_chip, EXAMPLE_QUESTIONS
# from ui.result_cards import build_result_panel
# from partner_solutions.registry import build_primary_registry
...
#         {"name": "description", "content": "Partner Command Center — live EULER MCP partner intelligence"},
#     ],
# )
...
#                 className="pc-brand",
#                 children=[
#                     html.Div("E", className="pc-brand-mark"),
#                     html.Div(
#                         [
#                             html.H1("Partner Command Center", className="pc-brand-title"),
#                             html.P(
#                                 [
#                                     html.Span("Claude", className="pc-brand-accent-violet"),
#                                     " · ",
#                                     html.Span("EULER MCP", className="pc-brand-accent-blue"),
#                                     " · Live partner intelligence",
#                                 ],
...
#     ),
#     className="header-banner pc-header",
# )
...
# # ─────────────────────────────────────────────────────────────────────────────
# # Real tool names from https://mcp.eulerapp.com/public/partner-capabilities
# # Deals are NOT "company_deals" — use partner_artifacts(action=deals) or get_search_deals.
# SCHEMA_PANEL = html.Details(
#     [
#         html.Summary("EULER MCP tools (documented reference)"),
#         html.Div(
#             [
#                 html.P([html.B("list_accounts"), " — Accounts this session can access"]),
#                 html.P([html.B("performance"), " — Partner metrics over a date range"]),
#                 html.P([html.B("commissions"), " — Commission earnings + deal breakdown"]),
#                 html.P([html.B("partner_artifacts"), " — deals / invoices / charges / agreements / tracking_links"]),
#                 html.P([html.B("get_search_deals"), " — Find a specific deal by name"]),
#                 html.P([html.B("referrals"), " — Referrals and deal registrations"]),
#                 html.P([html.B("content_list"), " · ", html.B("content_search"), " — Shared customer content"]),
#                 html.P([html.B("partner_users"), " — Team users and roles"]),
#                 html.P([html.B("partners"), " · ", html.B("partner_directory_search"), " — Admin partner list / directory"]),
#             ],
#             className="pc-schema-list",
#         ),
#     ],
...
#             [tab_dbgpt],
#             id="main-tabs",
#             active_tab="tab-dbgpt",
#             className="pc-tabs",
#         ),
#         html.Footer(
#             [
#                 html.Span("Partner Command Center"),
#                 html.Span(" · ", className="pc-footer-sep"),
#                 html.Span("Claude", className="pc-brand-accent-violet"),
#                 html.Span(" · ", className="pc-footer-sep"),
#                 html.Span("EULER MCP", className="pc-brand-accent-blue"),
#                 html.Span(" · live tools only — no mock catalog in routing"),
#             ],
#             className="pc-footer",
#         ),
#     ],
...
#     """Prefer the same live MCP client the agent uses."""
#     live_tools = []  # list of dicts: {name, description}
#     reason = ""

#     # 1) Same path as Groq agent
#     try:
...
#     # 2) Fallback: partner solution discover
#     if not live_tools:
#         try:
...
#             ]
#             live_tools = [t for t in live_tools if t.get("name")]
#             if live_tools:
#                 reason = ""
#         except Exception as exc:
#             if not reason:
...
#                 "write"
#                 if any(
#                     w in str(t["name"]).lower()
#                     for w in ("submit", "manage", "create", "update", "delete")
#                 )
#                 else "read",
#                 live=True,
#             )
...
#     # Offline: documented reference only
...
#         return {"display": "none"}, {"display": "inline-flex"}, clear_hash
#     return {"display": "inline-flex"}, {"display": "none"}, no_update
...
#     return f"<html><head><meta http-equiv='refresh' content=\"0;url=/#euler_code={safe_code}&euler_state={safe_state}\"></head><body>Redirecting to app...</body></html>"
...
# from utils.euler_oauth import build_auth_url, exchange_code, is_connected, get_token_set
...
#     ],
#     suppress_callback_exceptions=True,
#     title="Partner Network — Claude AI",
#     meta_tags=[
#         {"name": "viewport", "content": "width=device-width, initial-scale=1"},
#         {"name": "description", "content": "Partner Network — Live partner intelligence powered by Claude AI and EULER"},
#     ],
# )
# server = app.server  # Expose Flask server for WSGI deployment
...
#                 ]
#             ),
#             dbc.Button("🔌 Connect to EULER", id="euler-connect-btn", color="primary", size="sm", className="ms-auto", style={"fontWeight": "600"}),
#         ],
...
#             [
#                 "Partner Network — Claude AI · ",
#                 html.Span("Claude Sonnet",          style={"color": "#A78BFA"}), " · ",
#                 html.Span("EULER Live Partner Data", style={"color": "#2563EB"}), " · ",
#                 html.Span("MCP Tools",               style={"color": "#38BDF8"}),
#             ],
#             style={"textAlign": "center", "color": "#475569", "fontSize": "12px", "paddingBottom": "16px"},
#         ),
#     ],
#     fluid=True,
#     style={"background": "#0F172A", "minHeight": "100vh", "padding": "16px 28px"},
# )
...
#                 "background": "#38BDF8" if is_user else "#334155",
#                 "color": "#0F172A" if is_user else "#E2E8F0",
...
#     Output("dbgpt-history",     "data"),
#     Output("dbgpt-sql",         "children"),
#     Output("dbgpt-grid",        "columnDefs"),
#     Output("dbgpt-grid",        "rowData"),
#     Output("dbgpt-chart",       "figure"),
#     Output("dbgpt-status",      "children"),
#     Output("dbgpt-rag-context", "value"),
#     Output("chat-history-container", "children"),
#     Output("dynamic-charts-container", "children"),
#     Input("dbgpt-run-btn",  "n_clicks"),
#     State("dbgpt-input",    "value"),
#     State("dbgpt-history",  "data"),
#     State("rag-region",     "data"),
#     State("rag-tier",       "data"),
#     State("rag-source",     "data"),
#     prevent_initial_call=True,
#     running=[(Output("dbgpt-status", "children"), html.Div("LLM is thinking...", className="dbgpt-status-pending"), html.Div(""))],
# )
# def run_dbgpt(n_clicks, question, history, region, tier, source):
#     _empty_fig = go.Figure(layout=dict(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)"))
...
#     try:
#         result = tablerag_ask(question, history or [], region=region, tier=tier, source=source, model_provider="NIM")
#     except Exception:
#         _log_unexpected_error(question, region, tier, source)
#         raise
#     provider = result.get("provider") or "unknown"
...
#     Output("dbgpt-history",     "data",    allow_duplicate=True),
#     Output("dbgpt-sql",         "children",   allow_duplicate=True),
#     Output("dbgpt-grid",        "columnDefs", allow_duplicate=True),
#     Output("dbgpt-grid",        "rowData",    allow_duplicate=True),
#     Output("dbgpt-chart",       "figure",  allow_duplicate=True),
#     Output("dbgpt-status",      "children",allow_duplicate=True),
#     Output("dbgpt-rag-context", "value",   allow_duplicate=True),
#     Output("chat-history-container", "children", allow_duplicate=True),
#     Output("dynamic-charts-container", "children", allow_duplicate=True),
#     Output("dbgpt-input",       "value",   allow_duplicate=True),
#     Input("dbgpt-clear-btn", "n_clicks"),
#     prevent_initial_call=True,
# )
...
# @callback(
#     Output("euler-connect-btn", "children"),
#     Output("euler-connect-btn", "color"),
#     Output("url", "hash"),
#     Input("url", "hash"),
#     prevent_initial_call=False
# )
# def handle_hash(hash_val):
#     if hash_val and 'euler_code=' in hash_val:
#         params = dict(urllib.parse.parse_qsl(hash_val.lstrip('#')))
#         if 'euler_code' in params:
#             try:
#                 exchange_code(params['euler_code'], params.get('euler_state'))
#                 return "✅ EULER Connected", "success", ""
#             except Exception as e:
#                 print(f"OAuth error: {e}")
#                 return "❌ Connection Failed", "danger", ""
                
#     connected = is_connected()
#     text = "✅ EULER Connected" if connected else "🔌 Connect to EULER"
#     color = "success" if connected else "primary"
#     return text, color, no_update
